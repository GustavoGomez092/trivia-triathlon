...
const GamePositionTable = () => {
  const { scores } = useTopUsersForEvent('sprint');
  const { user } = useCurrentUser();

  return (
    <div className="nes-container is-rounded flex h-fit flex-col items-center gap-4 bg-gray-200 p-8">
      <h1 className="text-2xl font-bold">Event Stats</h1>
      <Table>
        <thead>
          <tr>
            {TABLE_HEADERS?.map(({ key, title }) => <th key={key}>{title}</th>)}
          </tr>
        </thead>
